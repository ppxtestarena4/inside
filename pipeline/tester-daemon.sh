#!/usr/bin/env bash
# tester-daemon.sh — Агент-тестировщик (Tester) для проекта Inside
...
# Запускается через systemd (inside-tester.service).
...
Автоматически созданный Pull Request по результатам прохождения всего конвейера Inside.

**Реализует:** #${issue_number} — ${issue_title}

## Чеклист конвейера

- [x] Coder (Claude): реализация завершена
- [x] Reviewer (Codex): код прошёл ревью
- [x] Tester (Codex): все тесты пройдены
...
process_testing() {
    ...
    if ! git ls-remote --exit-code --heads origin "${branch_name}" > /dev/null 2>&1; then
        ...
        comment_on_issue "${issue_number}" "❌ **Tester (Codex)**: ветка \`${branch_name}\` не найдена. Невозможно выполнить тестирование."
...
Ты — опытный QA-тестировщик проекта Inside. Проверь реализацию на соответствие спецификации.
...
PROMPT
    ...
    if [[ ${codex_exit} -eq 124 ]]; then
        ...
        comment_on_issue "${issue_number}" "❌ **Tester (Codex)**: превышен таймаут тестирования (15 мин). Задача возвращена в In Progress."
        move_issue_to_status "${item_id}" "In Progress"
        unassign_issue "${issue_number}"
        return 1
    elif [[ ${codex_exit} -ne 0 ]]; then
        log "ERROR: codex exec завершился с кодом ${codex_exit}"
        comment_on_issue "${issue_number}" "❌ **Tester (Codex)**: ошибка при выполнении тестирования (exit ${codex_exit}). Задача возвращена в In Progress."
        move_issue_to_status "${item_id}" "In Progress"
        unassign_issue "${issue_number}"
        return 1
    fi
    ...
    if [[ "${verdict}" == "PASS" ]]; then
        ...
        comment_on_issue "${issue_number}" "✅ **Tester (Codex)**: все тесты пройдены!
...
Задача перемещена в **Done**. Ожидается финальная проверка человеком."
        ...
        comment_on_issue "${issue_number}" "🔄 **Tester (Codex)**: тестирование не пройдено. Требуются исправления.
...
main() {
    check_dependencies
    log "============================================"
    log "Inside Tester Daemon запущен. Репозиторий: ${PIPELINE_REPO}"
