#!/usr/bin/env bash
# reviewer-daemon.sh — Агент-ревьюер (Reviewer) для проекта Inside
...
# Запускается через systemd (inside-reviewer.service).
...
process_review() {
    ...
    if ! git ls-remote --exit-code --heads origin "${branch_name}" > /dev/null 2>&1; then
        ...
        comment_on_issue "${issue_number}" "❌ **Reviewer (Codex)**: ветка \`${branch_name}\` не найдена. Невозможно выполнить ревью."
    ...
    if [[ -z "${changed_files}" ]]; then
        ...
        comment_on_issue "${issue_number}" "❌ **Reviewer (Codex)**: в ветке \`${branch_name}\` нет изменений относительно main. Возвращаем задачу в In Progress."
        move_issue_to_status "${item_id}" "In Progress"
        unassign_issue "${issue_number}"
        return 1
    fi
...
Ты — опытный QA-инженер и технический ревьюер проекта Inside. Проведи code review.
...
PROMPT
    ...
    if [[ ${codex_exit} -eq 124 ]]; then
        ...
        comment_on_issue "${issue_number}" "❌ **Reviewer (Codex)**: превышен таймаут ревью (15 мин). Задача возвращена в In Progress."
        move_issue_to_status "${item_id}" "In Progress"
        unassign_issue "${issue_number}"
        return 1
    elif [[ ${codex_exit} -ne 0 ]]; then
        log "ERROR: codex exec завершился с кодом ${codex_exit}"
        comment_on_issue "${issue_number}" "❌ **Reviewer (Codex)**: ошибка при выполнении ревью (exit ${codex_exit}). Задача возвращена в In Progress."
        move_issue_to_status "${item_id}" "In Progress"
        unassign_issue "${issue_number}"
        return 1
    fi
    ...
    if [[ "${verdict}" == "PASS" ]]; then
        ...
        comment_on_issue "${issue_number}" "✅ **Reviewer (Codex)**: код прошёл ревью.
...
Задача перемещена в **Testing**."
        ...
        comment_on_issue "${issue_number}" "🔄 **Reviewer (Codex)**: код не прошёл ревью. Требуются исправления.
...
main() {
    check_dependencies
    log "============================================"
    log "Inside Reviewer Daemon запущен. Репозиторий: ${PIPELINE_REPO}"
